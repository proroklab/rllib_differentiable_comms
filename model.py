...
class Model(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        self.action_shared = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(64, self.cfg['shared_nn_out_features_per_agent']*share_n_agents),
            nn.ReLU()
        )
        ...
        self.value_encoder_state = nn.Sequential(
            SlimConv2d(2, 8, 3, 2, 1), # in_channels, out_channels, kernel, stride, padding
            SlimConv2d(8, 8, 3, 2, 1), # in_channels, out_channels, kernel, stride, padding
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        if self.cfg['share_observations']:
            ...
            for i in range(self.n_agents):
                action_shared_features[:, i] = self.action_shared(action_feature_map[:, i])

        value_shared_features = self.value_shared(
            torch.cat([
                value_feature_map.view(batch_size, self.n_agents*self.cfg['encoder_out_features']),
